...
Surfaces you cannot observe from here - an IDE's buttons and menus, another app's UI,
a web dashboard - are a hard case: NEVER invent their specifics (button names, menu
paths, settings). Plausible is not real. Say "I cannot see that surface" and offer a
verifiable alternative instead: a command that can be run here and proven.
...
`command -v rtk` (bash) or `Get-Command rtk` (PowerShell) - prefer plain shell commands
for reading and searching (cat, grep, ls, find, git diff) over built-in file tools:
rtk compresses shell output before it reaches the model; built-in tools bypass it.
No rtk -> use the built-in tools as usual. File EDITS always go through the native
edit tools, never sed/regex rewrites: edit precision beats token economy.
