...
#
# Observes recognizable direct test commands and their tool outcomes. This is a lower-bound
# activity journal, not proof of test coverage, an authentic test script, or a trusted receipt.
# Compound commands, wrappers and dynamic shell syntax are omitted: their overall status
# cannot safely be attributed to a test. It blocks nothing and adds no agent workflow step.
#
# A line records the recognized command, reported tool outcome and partial content identity.
# It cannot certify a full suite or the exact tested tree (untracked files are not hashed).
#
...
# Deliberately not a shell interpreter. A supported observation is ONE simple command
# (optionally after the literal cd && above). Even a harmless quoted operator is omitted;
# false negatives are preferable to attributing another command's success to the tests.
...
if not words:
    bail()
nonruns = {"--help", "-h", "--version", "--collect-only", "--co", "--no-run",
           "--list", "--listtests", "--dry-run"}
relocates = {"--prefix", "--cwd", "--directory", "--chdir", "-C"}
if any(w.split("=", 1)[0].lower() in nonruns or w.split("=", 1)[0] in relocates
       for w in words[1:]):
    bail()
...
if any(c in root for c in "\t\r\n"):
    bail()                     # cannot be represented in the journal's TSV fields
...
expected_words = shlex.split(expected)
prefix = expected_words[:-1] if expected == "go test ./..." else expected_words
matched = expected if words[:len(prefix)] == prefix else None
scope = "full" if words == expected_words else "partial"
...
if not matched and expected in ("npm test", "pnpm test", "yarn test", "bun test"):
    for alt in ("npx vitest", "npx jest", "npx mocha", "node --test",
                "vitest", "jest", "mocha"):
        alt_words = shlex.split(alt)
        if words[:len(alt_words)] == alt_words:
...
# covered. This is not a complete snapshot or sufficient proof for reusing a test result.
worktree = git("hash-object", "--stdin", stdin=diff) or "no-hash"

stamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
line = "\t".join((stamp, outcome, scope, root, normalised, head[:12], worktree[:12], "v2"))
